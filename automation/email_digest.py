...
def get_today_observations():
    try:
        with get_db_connection() as conn:
            ...
            c.execute("SELECT MAX(timestamp) as latest FROM observations")
            latest_row = c.fetchone()
            if latest_row and latest_row['latest']:
                try:
                    latest_dt = datetime.fromisoformat(latest_row['latest'].replace('Z', '+00:00'))
                    cutoff_time = latest_dt - timedelta(days=1)
                except Exception:
                    cutoff_time = datetime.now(timezone.utc) - timedelta(days=1)
            else:
                cutoff_time = datetime.now(timezone.utc) - timedelta(days=1)
                
            c.execute("SELECT timestamp, headline FROM observations WHERE timestamp >= %s ORDER BY timestamp DESC LIMIT 10", (cutoff_time.isoformat().replace('+00:00', 'Z'),))
            return c.fetchall()
    except Exception as e:
