...
def seed_all_empty_tables(db_path=None, quiet=False):
    ...
    if c.fetchone()[0] == 0:
        ...
        for pid, cid, weight, cost, notes in positions:
            c.execute("INSERT INTO portfolio_positions (portfolio_id, company_id, weight_pct, cost_basis, notes) VALUES (?, ?, ?, ?, ?)", (pid, cid, weight, cost, notes))
        for pid, div, conc, sc, corr, stress, comp in [(pids[0], 78.5, 5.2, 3.8, 2.1, -4.5, 72.3), (pids[1], 65.0, 8.5, 6.2, 4.8, -8.2, 56.8), (pids[2], 85.0, 2.5, 1.8, 1.2, -2.8, 82.2)]:
            c.execute("INSERT INTO portfolio_scores (portfolio_id, diversification_score, concentration_penalty, sector_concentration_penalty, correlation_penalty, stress_impact_score, composite_score) VALUES (?, ?, ?, ?, ?, ?, ?)", (pid, div, conc, sc, corr, stress, comp))
        for scenario, impact_pct, impact_val, max_pos, num_aff, total in [('Market crash -20%', -14.2, -8500000, 'RELIANCE:-18.5%', 8, 10), ('Rate hike +100bps', -6.8, -4100000, 'HDFCBANK:-9.2%', 5, 10), ('Oil price shock +30%', -3.5, -2100000, 'RELIANCE:-7.5%', 4, 10), ('Currency depreciation -5%', -4.2, -2500000, 'INFY:-8.1%', 6, 10)]:
            c.execute("INSERT INTO portfolio_stress_results (portfolio_id, scenario, impact_pct, impact_value, max_position_impact, num_positions_affected, total_positions) VALUES (?, ?, ?, ?, ?, ?, ?)", (pids[0], scenario, impact_pct, impact_val, max_pos, num_aff, total))
    ...
    if c.fetchone()[0] == 0:
        ...
        for spid, trade_date, trade_type, ticker, price, qty, reason in [
            ...
            c.execute("INSERT INTO shadow_trades (portfolio_id, trade_date, trade_type, ticker, price, quantity, reason) VALUES (?, ?, ?, ?, ?, ?, ?)", (spid, trade_date, trade_type, ticker, price, qty, reason))
    ...
    if c.fetchone()[0] == 0:
        for cid, title, thesis_text, key_vars, timeframe, conviction, status, notes in [
            ...
            c.execute("INSERT INTO theses (company_id, title, thesis_text, key_variables, timeframe_days, conviction, status, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (cid, title, thesis_text, key_vars, timeframe, conviction, status, notes))
        ...
        for t in all_theses[:5]:
            for var, expected, actual, severity, notes in [('AUM Growth', '22-28%', '25.3%', 'GREEN', 'On track'), ('NIM', '4.5-5.0%', '4.8%', 'GREEN', 'Stable'), ('Credit Cost', '<1.5%', '1.2%', 'GREEN', 'Better than expected')]:
                c.execute("INSERT INTO thesis_checks (thesis_id, variable, expected_range, actual_value, flag_severity, notes) VALUES (?, ?, ?, ?, ?, ?)", (t['id'], var, expected, actual, severity, notes))
    ...
    if c.fetchone()[0] == 0:
        for cid, analysis_date, prior_date, category, prior_obs, current_obs, status, magnitude, evidence in [
            ...
            c.execute("""INSERT INTO thesis_evolution (company_id, analysis_date, prior_analysis_date, category, prior_observation, current_observation, evolution_status, magnitude, evidence) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""", (cid, analysis_date, prior_date, category, prior_obs, current_obs, status, magnitude, evidence))
        for cid, scored_at, bq, ca, gov, liq, fs, me, val, direction, summary in [
            ...
            c.execute("""INSERT INTO thesis_scorecard (company_id, scored_at, business_quality, capital_allocation, governance, liquidity, funding_structure, macro_exposure, valuation, overall_direction, scorecard_summary) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (cid, scored_at, bq, ca, gov, liq, fs, me, val, direction, summary))
    ...
    if c.fetchone()[0] == 0:
        for ticker, company, otype, headline, severity, supporting_data, regime_relevance in [
            ...
            c.execute("INSERT INTO observations (ticker, company, type, headline, severity, supporting_data, regime_relevance) VALUES (?, ?, ?, ?, ?, ?, ?)", (ticker, company, otype, headline, severity, supporting_data, regime_relevance))
    ...
    if c.fetchone()[0] == 0:
        for cid, filing_type, period, source_url, local_path, status in [
            ...
            c.execute("INSERT INTO filings (company_id, filing_type, period, source_url, local_path, status) VALUES (?, ?, ?, ?, ?, ?)", (cid, filing_type, period, source_url, local_path, status))
        log("Seeded filings")
    ...
    if c.fetchone()[0] == 0:
        for cid, period, risk, conf, regime_sens, struct_qual, comp, rationale in [
            ...
            c.execute("INSERT INTO institutional_scores (company_id, period, risk_intensity, confidence, regime_sensitivity, structural_quality, composite_score, scoring_rationale) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (cid, period, risk, conf, regime_sens, struct_qual, comp, rationale))
    ...
    if c.fetchone()[0] == 0:
        for name, metric, category, total_obs, confirmed, invalidated, acc_rate, avg_conf, pred_val, rank in [
            ...
            c.execute("""INSERT INTO edge_discovery_framework (framework_name, metric_name, category, total_observations, confirmed_count, invalidated_count, accuracy_rate, avg_confidence, predictive_value, rank_score) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (name, metric, category, total_obs, confirmed, invalidated, acc_rate, avg_conf, pred_val, rank))
