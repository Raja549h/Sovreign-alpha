...
try:
    from research.evolution_quality import (
        EvidenceTimeline, ConfidenceCalibrator,
        WeeklyICReport
    )
    ...
    with app.app_context():
        ...
        try:
            ic = WeeklyICReport()
            sections = ic.generate_report()
            log("RESEARCH", "WeeklyICReport.generate_report()", "PASS", f"sections={len(sections.get('sections',{}))}")
        except Exception as e:
            log("RESEARCH", "WeeklyICReport.generate_report()", "FAIL", str(e)[:80])
...
try:
    ...
    with app.app_context():
        ...
        try:
            obs = registry.get_registry(limit=5)
            log("REGISTRY", "get_registry()", "PASS", f"{len(obs)} records")
        except Exception as e:
            log("REGISTRY", "get_registry()", "FAIL", str(e)[:80])
        try:
            score = registry.calculate_edge_score()
...
try:
    ...
    log("FII", "FIIIntelligence import", "FAIL", str(e)[:80])
